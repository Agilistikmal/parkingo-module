...
try:
    ...
    print(
        "[ℹ️ INFO] Please install tesseract: sudo pacman -S tesseract tesseract-data-eng"
    )
    exit(1)
...
try:
    session = ort.InferenceSession(
        "./data/best.onnx", providers=["CPUExecutionProvider"]
    )
    ...
    try:
        ...
        for i in range(rows):
            ...
            if confidence > conf_threshold:
                ...
                if (
                    ...
                    and MIN_PLATE_AREA <= relative_area <= MAX_PLATE_AREA
                ):
        ...
        if len(final_boxes) > 1:
            sorted_indices = sorted(
                range(len(final_boxes)), key=lambda k: final_boxes[k][1]
            )
        ...
        print(f"Failed to postprocess: {e}")
        return [], []
...
def preprocess_plate(plate_region):
    ...
    try:
        ...
        _, thresh_normal = cv2.threshold(
            gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU
        )
        _, thresh_inverse = cv2.threshold(
            gray, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU
        )

        # 5. Add white border to both
        border_size = 20
        thresh_normal = cv2.copyMakeBorder(
            thresh_normal,
            border_size,
            border_size,
            border_size,
        ...
        thresh_inverse = cv2.copyMakeBorder(
            thresh_inverse,
            border_size,
            border_size,
            border_size,
        ...
        print(f"Failed to preprocess plate: {e}")
        return [plate_region]
...
def read_license_plate(frame, box):
    try:
        ...
        height = int(h * scale)
        resized = cv2.resize(
            plate_region, (width, height), interpolation=cv2.INTER_CUBIC
        )
        ...
        for processed in processed_images:
            for psm in psm_modes:
                ...
                text = pytesseract.image_to_string(
                    processed, config=config, lang="eng"
                ).strip()
                if text:
                    results.append(text)
        ...
        print(f"Failed to read plate: {e}")
        return ""
...
def fix_common_errors(text):
    ...
    try:
        ...
        print(f"Failed to fix common errors: {e}")
        return text
...
def clean_license_plate(text):
    try:
        ...
        print(f"Failed to clean plate text: {e}")
        return ""
...
def scan(frame: cv2.typing.MatLike) -> str:
    try:
        ...
        boxes, confidences = postprocess(
            outputs[0], frame.shape, conf_threshold=0.2, iou_threshold=0.4
        )
        ...
        print(f"Error in plate scanning: {e}")
        return ""
